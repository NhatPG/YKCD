<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Province.Master" AutoEventWireup="true" CodeBehind="ThongKe_TheoDonVi.aspx.cs" Inherits="YKCD.Province.Application.Province.ThongKe_TheoDonVi" %>
        ...
        <div class="col-md-12">
            <div class="form-inline text-center">
                <div class="form-group">
                    ...
                    <asp:DropDownList ID="NhomDonVi" CssClass="form-control input-sm select2" runat="server"></asp:DropDownList>
                </div>
                <div class="form-group">
                    <label>Từ ngày: </label>
    ...
    <asp:ListView ID="lvData" runat="server">
        ...
        <ItemTemplate>
            <tr>
                ...
                <td class="text-left">
                    <strong>
                        <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "fromDate", TuNgay.Text.Replace("/", "-")) %>"><%# Eval("ObjectName") %></a>
                    </strong>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienTrongHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("NotPerformInTerm").ToInteger() > 0 ? Eval("NotPerformInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienQuaHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("NotPerformOutTerm").ToInteger() > 0 ? Eval("NotPerformOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.DangThucHienTrongHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("PerformingInTerm").ToInteger() > 0 ? Eval("PerformingInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.DangThucHienQuaHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("PerformingOutTerm").ToInteger() > 0 ? Eval("PerformingOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.ChoXacNhan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("WaitToConfirm").ToInteger() > 0 ? Eval("WaitToConfirm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.DaThucHienDungHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("DoneInTerm").ToInteger() > 0 ? Eval("DoneInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Province.DanhSachYKCD, ParameterNames.AgencyID, Eval("ObjectID"), "status", TrangThai.DaThucHienTreHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
