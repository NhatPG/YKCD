<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ThongTinYKienChiDaoControl.ascx.cs" Inherits="YKCD.SubAgency.Application.Controls.ThongTinYKienChiDaoControl" %>
<fieldset>
...
<asp:Repeater ID="RequestDetail" runat="server">
    <ItemTemplate>
        <table class="table table-data table-info">
            ...
            <tr style="<%#Eval("IsAgencyRequest").ToBoolean() ? "": "display: none" %>">
                <td class="nowrap">
            ...
            <tr>
                <td colspan="6" class="text-right">
                    <a href="<%# Redirector.GetLink(ViewNames.SubAgency.BaoCao_QuanLy, "pid", Eval("RequestID")) %>"
                       class="btn btn-primary btn-sm" style="<%# Eval("CoQuyenBaoCao").ToBoolean() ? "": "display: none" %>">
                        <span class="glyphicon glyphicon-plus-sign" aria-hidden="true"></span>
                        Nhập báo cáo
                    </a>
                    <a href="<%# Redirector.GetLink(ViewNames.SubAgency.YKCD_GiaoViec, "pid", Eval("RequestID")) %>"
                       class="btn btn-success btn-sm" style="<%# Eval("IsAgencyRequest").ToBoolean() && !Eval("IsAssignPerform").ToBoolean() && Authenticator.CheckRole(UserRole.ChuyenVienVP, UserRole.LanhDaoVP, UserRole.LanhDaoDonVi, UserRole.Administrator) ? "": "display: none" %>">
                        <span class="glyphicon glyphicon-share" aria-hidden="true"></span>
                        Giao việc
                    </a>
                    <a href="<%# Redirector.GetLink(ViewNames.SubAgency.YKCD_QuanLy, "id", Eval("RequestID")) %>"
