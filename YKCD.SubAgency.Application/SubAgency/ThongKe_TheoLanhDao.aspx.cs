...
namespace YKCD.SubAgency.Application.SubAgency
{
    public partial class ThongKe_TheoLanhDao : ListViewPageBase
    {
        ...
        protected DocumentModel CreateDocument()
        {
            ...
            document.Content.Find("(TieuDe)")?.First()?.LoadText($"BÁO CÁO TỔNG HỢP TÌNH HÌNH THỰC HIỆN Ý KIẾN CHỈ ĐẠO CỦA {AppSettings.AGENCY_NAME.ToUpper()}");
            document.Content.Find("(ThoiDiemThongKe)")?.First()?.LoadText($"Thống kê từ ngày {TuNgay.Text} đến ngày {DenNgay.Text}");
            document.Content.Find("(TenDonVi)")?.First().LoadText($"{AppSettings.AGENCY_NAME.ToUpper()}");
