<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Agency.Master" AutoEventWireup="true" CodeBehind="ThongKe_TheoLanhDao.aspx.cs" Inherits="YKCD.Agency.Application.Agency.ThongKe_TheoLanhDao" %>
    ...
    <asp:ListView ID="lvData" runat="server">
        ...
        <ItemTemplate>
            <tr>
                ...
                <td class="text-left">
                    <strong>
                        <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "fromDate", TuNgay.Text.Replace("/", "-")) %>"><%# Eval("ObjectName") %></a>
                    </strong>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienTrongHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("NotPerformInTerm").ToInteger() > 0 ? Eval("NotPerformInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienQuaHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("NotPerformOutTerm").ToInteger() > 0 ? Eval("NotPerformOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.DangThucHienTrongHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("PerformingInTerm").ToInteger() > 0 ? Eval("PerformingInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.DangThucHienQuaHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("PerformingOutTerm").ToInteger() > 0 ? Eval("PerformingOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.ChoXacNhan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("WaitToConfirm").ToInteger() > 0 ? Eval("WaitToConfirm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.DaThucHienDungHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
                        <%# Eval("DoneInTerm").ToInteger() > 0 ? Eval("DoneInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Agency.DanhSachYKCD, ParameterNames.PresidentID, Eval("ObjectID"), "status", TrangThai.DaThucHienTreHan, "fromDate", TuNgay.Text.Replace("/", "-")) %>">
