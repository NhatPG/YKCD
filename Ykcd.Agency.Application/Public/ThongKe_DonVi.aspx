<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Public.Master" AutoEventWireup="true" CodeBehind="ThongKe_DonVi.aspx.cs" Inherits="YKCD.Agency.Application.Public.ThongKe_DonVi" EnableViewState="false" %>
    ...
    <asp:ListView ID="lvData" runat="server">
        ...
        <ItemTemplate>
            <tr>
                ...
                <td class="text-left">
                    <strong>
                        <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID")) %>"><%# Eval("ObjectName") %></a>
                    </strong>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienTrongHan) %>">
                        <%# Eval("NotPerformInTerm").ToInteger() > 0 ? Eval("NotPerformInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.ChuaThucHienQuaHan) %>">
                        <%# Eval("NotPerformOutTerm").ToInteger() > 0 ? Eval("NotPerformOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.DangThucHienTrongHan) %>">
                        <%# Eval("PerformingInTerm").ToInteger() > 0 ? Eval("PerformingInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.DangThucHienQuaHan) %>">
                        <%# Eval("PerformingOutTerm").ToInteger() > 0 ? Eval("PerformingOutTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.ChoXacNhan) %>">
                        <%# Eval("WaitToConfirm").ToInteger() > 0 ? Eval("WaitToConfirm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-in-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.DaThucHienDungHan) %>">
                        <%# Eval("DoneInTerm").ToInteger() > 0 ? Eval("DoneInTerm").ToString() : "-" %>
                    </a>
                </td>
                <td class="text-out-term">
                    <a href="<%# Redirector.GetLink(ViewNames.Public.DanhSachYKCD, ParameterNames.DepartmentID, Eval("ObjectID"), "status", TrangThai.DaThucHienTreHan) %>">
